...
def train_recon(train_loader, writer, epoch, g_model, g_optm, g_lr):
...
def train(train_loader, writer, epoch, g_model, g_optm, g_lr, d_model, d_optm, d_lr):
    ...
    for data in train_loader:
        ...
        # gradient_penalty = compute_gradient_penalty(d_model, hdCT, img_net)
...
if __name__ == "__main__":
    ...
    method = 'clear_with_gan_multi_step'
    batch_size = 2
    ...
    scheduler_g = torch.optim.lr_scheduler.MultiStepLR(optimizer_g, milestones=[5, 100], gamma=0.1)
    scheduler_d = torch.optim.lr_scheduler.MultiStepLR(optimizer_d, milestones=[100, 200], gamma=0.1)

    if os.path.exists(save_dir + 'G/') is False:
    ...
    if os.path.exists(save_dir + 'D/') is False:
    ...
    for epoch in range(15, 100):
        ...
        if epoch<15:
            ...
            train_recon(train_loader, writer, epoch, generator, optimizer_g, scheduler_g)
