...
class clear_generator(nn.Module):

    def __init__(self, chl=16):
        super(clear_generator, self).__init__()
        self.chl = chl

        self.net = nn.ModuleList()
        self.net = self.net.append(UNet3D(in_chl=1, out_chl=1, model_chl=self.chl))
        self.net = self.net.append(UNet3D(in_chl=1, out_chl=1, model_chl=self.chl))
    ...
    def forward(self, proj):
        ...
        # print(proj_net.size())
        img_fbp = recon_ops_example.backprojection(
            recon_ops_example.filter_sinogram(proj_net, filter_name="ramp")) * 1024
        # print(img_fbp.size())
...
class DDImgCLEAR(nn.Module):

    def __init__(self, chl=32):
        super(DDImgCLEAR, self).__init__()
        self.chl = chl
    ...
    def forward(self, ldct):
        ...
        img_net1 = self.net[0](ldct)
        img_net2 = self.net[1](img_net1)

        return img_net2
    

class DDProjCLEAR(nn.Module):

    def __init__(self, chl=32):
        super(DDProjCLEAR, self).__init__()
        self.chl = chl
    ...
    def forward(self, proj):
        ...
        proj_net1 = self.net[0](proj)
        proj_net2 = self.net[1](proj_net1)
        
        img_fbp = recon_ops_example.backprojection(recon_ops_example.filter_sinogram(proj_net2)) * 1024

        return proj_net2, img_fbp
